...
- Node.js 18+ and npm/pnpm/bun installed
...
`@cloudflare/next-on-pages` adapter to compile the Next.js app into the
Cloudflare Pages format (static assets + Edge Functions for API routes).

The API routes run on Cloudflare's Edge runtime (V8 isolate), so we replaced
the Node-only `soundcloud.ts` library with an Edge-compatible client
(`src/lib/soundcloud-edge.ts`) that uses `fetch` directly. This means **no
Node.js polyfills needed** — it works natively on Cloudflare's Edge.
